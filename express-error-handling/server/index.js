...
app.post('/api/grades', (req, res, next) => {
  ...
  if (!Number.isInteger(score) || score < 0 || score > 100) {
    throw new ClientError(400, 'score must be a integer between 0 and 100');
  }
  const sql = `
...
app.get('/api/grades/:gradeId', (req, res, next) => {
  ...
  if (!Number.isInteger(gradeId) || gradeId < 1) {
    throw new ClientError(400, 'grade must be a positive');
  }
  const sql = `
    select *
      from "grades"
  ...
  db.query(sql, params)
    .then(result => {
      ...
      if (!grade) {
        throw new ClientError(404, `cannot find grade with grade Id ${gradeId}`);
      } else {
        res.json(grade);
      }
    })
